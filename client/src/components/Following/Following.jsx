    ...
    const handleCreateChat = async (userId) => {
        try {
          ...
          if (existingChat.data !== null) {
            ...
            console.log(`Redirecting to existing chat with ID: ${existingChat._id}`);
            setHasChat(true);
          } else {
            // If a chat doesn't exist, create a new chat
            const { data } = await createChat({ senderId: user._id, receiverId: userId });
            console.log(data);
    ...
    <div className='drawer-container'>
        ...
        <div className='drawer-body'>
            {followingDetails.map((person) => (
                <div onClick={() => handleCreateChat(person.data._id)} className='drawer-item' key={person.data._id}>
                    <img
                   ...
                   <div className='drawer-infor'>
                        <span className='drawer-name'>{person.data.firstname} {person.data.lastname}</span>
